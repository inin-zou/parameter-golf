"""Nemotron-H inspired Mamba-3 Hybrid for Parameter Golf — with DSQ quantization.
...
DSQ (Decoupled Scale Quantization) additions:
- Mamba-aware quantization: SSM layers use lower bit width (int4) vs attention (int6)
- Per-channel-group scales for SSM projection weights (groups of 64)
- Smoothing factor to reduce outlier impact before quantization
- Controlled via USE_DSQ=1, SSM_QUANT_BITS, ATTN_QUANT_BITS, DSQ_GROUP_SIZE env vars
"""
...
def main() -> None:
    ...
    if args.use_gptq and master_process:
        ...
        for name, t in sd.items():
            ...
            layer_bits = get_layer_quant_bits(name, args)
